...
window.NFEvaluators = (function() {
    ...
    // Get evaluators list - Support both Firebase and Supabase
    async function getEvaluators() {
        // Try Supabase first - use async client getter
            ...
            try {
                ...
                if (error) {
                    ...
                    if (error.code === '42P01' || error.message?.includes('does not exist')) {
                        console.warn('Evaluators table does not exist - using defaults');
                ...
                console.error('Error fetching evaluators from Supabase:', error);
                return DEFAULT_EVALUATORS;
            }
        }
    ...
    // Add new evaluator - Support both Firebase and Supabase
    async function addEvaluator(evaluator) {
        ...
        if (client && window.currentUser) {
            try {
                ...
                if (error) {
                    // Check if table doesn't exist
                    if (error.code === '42P01' || error.message?.includes('does not exist')) {
                        throw new Error('جدول القائمين بالتقييم غير موجود. يرجى إنشاءه في Supabase.');
                    }
                    throw error;
                }
                
                if (window.NFActivity) {
                    NFActivity.log('EVALUATOR_ADDED', { evaluator: evaluator.name });
                }
                ...
                if (window.showNotification) {
                    ...
                    window.showNotification(`خطأ في إضافة المقيّم: ${errorMsg}`, 'error');
                }
                return null;
            }
        }
        ...
        if (!window.currentUser || !window.db) {
            console.warn('No Supabase client or Firebase available');
            if (window.showNotification) {
                window.showNotification('لم يتم تحميل النظام. يرجى تحديث الصفحة.', 'error');
            }
            return null;
        }
    ...
    // Delete evaluator - Support both Firebase and Supabase
    async function deleteEvaluator(evaluatorId) {
        ...
        if (client && window.currentUser) {
            try {
                ...
                if (window.NFActivity) {
                    NFActivity.log('EVALUATOR_DELETED', { evaluatorId });
                }
                
                return true;
            } catch (error) {
                console.error('Error deleting evaluator from Supabase:', error);
                return false;
            }
        }
...
window.NFWarehouseEnhanced = (function() {
    ...
    // 🔧 DEBUG flag for warehouse operations
    const DEBUG_WAREHOUSE = true;
    
    function logWarehouse(action, data) {
        if (DEBUG_WAREHOUSE) {
            console.log(`🏭 [Warehouse] ${action}:`, data);
        }
    }
    
    // Add warehouse with manager field - Support Supabase
    async function addWarehouse(warehouse) {
        logWarehouse('addWarehouse called', warehouse);
        ...
        if (!client && typeof window.getSupabaseClientAsync === 'function') {
            try {
                client = await window.getSupabaseClientAsync();
                logWarehouse('Got Supabase client via async', !!client);
            } catch (e) {
                console.warn('Could not get Supabase client:', e);
            }
        }
        ...
        if (client && window.currentUser) {
            logWarehouse('Using Supabase', { userId: window.currentUser.id });
            try {
                const warehouseData = {
                    name: warehouse.name,
                    ...
                    managerphone: warehouse.managerPhone || null,
                    user_id: window.currentUser.id,
                    created_at: new Date().toISOString()
                };
                
                logWarehouse('Inserting warehouse data', warehouseData);
                ...
                if (error) {
                    logWarehouse('Supabase insert error', error);
                    // Check if table doesn't exist
                    if (error.code === '42P01' || error.message?.includes('does not exist')) {
                        if (window.showNotification) {
                            window.showNotification('جدول المستودعات غير موجود في قاعدة البيانات. يرجى إنشاءه.', 'error');
                        }
                        throw new Error('جدول المستودعات غير موجود');
                    }
                    throw error;
                }
                
                logWarehouse('Warehouse added successfully', data);
                
                if (window.NFActivity) {
                    NFActivity.log('WAREHOUSE_ADDED', { 
                ...
                if (window.showNotification) {
                    const errorMsg = error.message || 'خطأ غير معروف';
                    window.showNotification(`خطأ في إضافة المستودع: ${errorMsg}`, 'error');
                }
                return null;
            }
        }
        ...
        if (!window.currentUser || !window.db) {
            logWarehouse('No client available', { supabase: !!client, firebase: !!window.db });
            if (window.showNotification) {
                window.showNotification('لم يتم تحميل النظام. يرجى تحديث الصفحة.', 'error');
            }
            return null;
        ...
        logWarehouse('Using Firebase fallback', null);
        try {
            const docRef = await db.collection('users')
                .doc(currentUser.uid)
    ...
    // Delete warehouse - Support Supabase
    async function deleteWarehouse(warehouseId) {
        logWarehouse('deleteWarehouse called', { warehouseId });
        ...
        if (client && window.currentUser) {
            logWarehouse('Using Supabase for delete', { warehouseId });
            try {
                // First check if any vehicles are in this warehouse
                const { data: vehiclesInWarehouse, error: checkError } = await client
                    .from('vehicles')
                    .select('id')
                    .eq('user_id', window.currentUser.id)
                ...
                if (checkError) {
                    logWarehouse('Error checking vehicles', checkError);
                }
                
                if (vehiclesInWarehouse && vehiclesInWarehouse.length > 0) {
                    logWarehouse('Cannot delete - vehicles exist', { count: vehiclesInWarehouse.length });
                    if (window.showNotification) {
                        window.showNotification(`لا يمكن حذف المستودع - يوجد ${vehiclesInWarehouse.length} مركبة مرتبطة به`, 'warning');
                    }
                    return false;
                }
                ...
                if (error) {
                    logWarehouse('Supabase delete error', error);
                    throw error;
                }
                
                logWarehouse('Warehouse deleted successfully', { warehouseId });
                
                if (window.NFActivity) {
                    NFActivity.log('WAREHOUSE_DELETED', { 
                        warehouseId, 
                        action: 'حذف مستودع'
                    });
                }
        ...
        // Fallback to Firebase (legacy)
        if (!window.currentUser || !window.db) {
            logWarehouse('No client available for delete', null);
            return false;
        }
        
        logWarehouse('Using Firebase fallback for delete', null);
        try {
            // First check if any vehicles are in this warehouse
    ...
    async function saveWarehouse() {
        logWarehouse('saveWarehouse called', null);
        ...
        logWarehouse('Form values', { name, location, capacity, manager, managerPhone });
        
        if (!name || !location) {
            logWarehouse('Validation failed', { name, location });
            if (window.showNotification) {
        ...
        logWarehouse('addWarehouse result', result);
        ...
        if (result) {
            ...
            logWarehouse('Refreshing warehouse page', null);
            if (typeof window.loadWarehousePage === 'function') {
                await window.loadWarehousePage();
            } else if (window.NFWarehouse?.refresh) {
                await window.NFWarehouse.refresh();
            }
        }
        // Error notification is now shown in addWarehouse function
    }
    ...
    async function executeDelete(warehouseId) {
        logWarehouse('executeDelete called', { warehouseId });
        
        document.getElementById('deleteWarehouseModal')?.remove();
        
        const result = await deleteWarehouse(warehouseId);
        
        logWarehouse('deleteWarehouse result', { warehouseId, result });
        
        if (result) {
            if (window.showNotification) {
                window.showNotification('تم حذف المستودع بنجاح', 'success');
            }
            
            // Refresh warehouse page
            logWarehouse('Refreshing warehouse page after delete', null);
            if (typeof window.loadWarehousePage === 'function') {
                await window.loadWarehousePage();
            } else if (window.NFWarehouse?.refresh) {
                await window.NFWarehouse.refresh();
...
window.NFExcelExport = (function() {
    ...
    // Export with GPS links and individual photo links (one column per photo)
    function exportAllToExcel(vehicles) {
        ...
        try {
            ...
            // Find the maximum number of images across all vehicles
            let maxImages = 0;
            vehicles.forEach(v => {
                if (v.images && Array.isArray(v.images)) {
                    // Filter only valid URLs (http/https)
                    const validImages = v.images.filter(img => img && typeof img === 'string' && img.startsWith('http'));
                    if (validImages.length > maxImages) {
                        maxImages = validImages.length;
                    }
                }
            });
            
            // Count total images for statistics
            let totalImages = 0;
            vehicles.forEach(v => {
                if (v.images && Array.isArray(v.images)) {
                    totalImages += v.images.filter(img => img && typeof img === 'string' && img.startsWith('http')).length;
                }
            });
            ...
            // Add dynamic photo columns based on max images
            const photoHeaders = [];
            for (let i = 1; i <= maxImages; i++) {
                photoHeaders.push(`صورة ${i}`);
            }
            ...
            // Get valid image URLs (only http/https)
            function getValidImageUrls(images) {
                if (!images || !Array.isArray(images)) return [];
                return images.filter(img => img && typeof img === 'string' && img.startsWith('http'));
            }
            
            // 🔒 إنشاء نسخة ثابتة من المركبات مع معرّف فريد لضمان عدم اختلاط البيانات
            const vehiclesSnapshot = vehicles.map((v, idx) => ({
                ...v,
                _exportIndex: idx,
                _exportId: v.id
            }));
            
            // 🔒 إنشاء خريطة للصور حسب معرف المركبة
            const vehicleImagesMap = new Map();
            vehiclesSnapshot.forEach(v => {
                vehicleImagesMap.set(v.id, {
            ...
            // Data rows
            const data = vehiclesSnapshot.map(v => {
                // Generate Google Maps URL
                let mapsUrl = '';
                ...
                // 🔒 استخدام الصور من الخريطة للتأكد من التطابق
                const vehicleData = vehicleImagesMap.get(v.id);
                const validImages = vehicleData ? vehicleData.images : [];
            ...
            const colWidths = [
                ...
                { wch: 50 }  // Open Map column
            ];
            
            // Add widths for image columns
            for (let i = 0; i < maxImages; i++) {
                colWidths.push({ wch: 60 });  // Wide columns for image URLs
            }
            ...
            // 🔒 استخدام vehiclesSnapshot بدلاً من vehicles للتأكد من التطابق مع البيانات
            vehiclesSnapshot.forEach((v, index) => {
                const rowNum = index + 2; // +2 because row 1 is headers (0-indexed becomes 1-indexed + header row)
                
                // 🔒 استخدام الصور من الخريطة المحفوظة مسبقاً
                const vehicleData = vehicleImagesMap.get(v.id);
                ...
                // 🔒 استخدام الصور من الخريطة لضمان التطابق 100%
                const validImages = vehicleData ? vehicleData.images : [];
                ...
                validImages.forEach((imgUrl, imgIndex) => {
                    ...
                    if (vehiclesSheet[cellRef]) {
                        // 🔒 تحقق من تطابق URL
                        const cellValue = vehiclesSheet[cellRef].v;
                        if (cellValue && cellValue !== imgUrl) {
                            console.error(`❌ [NFExport] Image mismatch at row ${rowNum}!`);
                            console.error(`   Vehicle: ${v.contractNo} (${v.make} ${v.model})`);
                        }
            ...
            console.log(`✅ [NFExport] Processed ${vehiclesSnapshot.length} vehicles with images map`);
            
            XLSX.utils.book_append_sheet(wb, vehiclesSheet, 'المركبات');
            
            // Log activity
            if (window.NFActivity) {
                NFActivity.log('EXPORT_EXCEL', { 
    ...
    console.log('📊 NFExcelExport initialized - Individual photo columns enabled');
